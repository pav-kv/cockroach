...
func TestFindConflict(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{sl: logSlice{}, want: ids[0]},
		// no conflict
		{sl: ids[0].append(1, 2, 3), want: ids[3]},
		{sl: ids[1].append(2, 3), want: ids[3]},
		{sl: ids[2].append(3), want: ids[3]},
		// no conflict, but has new entries
		{sl: ids[0].append(1, 2, 3, 4, 4), want: ids[3]},
		{sl: ids[1].append(2, 3, 4, 4), want: ids[3]},
		{sl: ids[2].append(3, 4, 4), want: ids[3]},
		{sl: ids[3].append(4, 4), want: ids[3]},
		// passes prev check, but conflicts with existing entries
		{sl: ids[0].append(4, 4), want: ids[0]},
		{sl: ids[1].append(1, 4, 4), want: ids[1]},
		{sl: ids[2].append(2, 2, 4, 4), want: ids[2]},
		// out of bounds
		...
		t.Run("", func(t *testing.T) {
			...
			match, ok := log.findConflict(tt.sl)
...
func TestHasNextCommittedEnts(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			if tt.snap {
				newSnap := snap
				newSnap.snap.Metadata.Index += 10
...
func TestNextCommittedEnts(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			raftLog := newLog(storage, discardLogger)
			require.True(t, raftLog.append(init))
			// Write the first unstable entry into storage, as if it was sent from the
			// raftLog after it was appended.
			require.NoError(t, storage.Append(init.entries[:1]))
