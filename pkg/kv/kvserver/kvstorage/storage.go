...
// NewBatch creates a new write batch to storage. If engines are separated, it
// consists of two batches, one per engine.
func (e *Engines) NewBatch() Batch {
	if !e.Separated() {
		// TODO(sep-raft-log): wrap for key assertions.
		b := e.Engine().NewBatch()
		return Batch{state: b, raft: b}
	}
	return Batch{
...
// Batch is a read-write batch to storage which is aware whether the raft/log
// and state machine engines are separated.
type Batch struct {
	state     storage.Batch
	raft      storage.Batch
	separated bool
...
// RW returns the separated ReadWriter for state machine and raft.
func (b *Batch) RW() (StateRW, RaftRW) {
	return b.state, b.raft
}

// CommitAndSync commits and syncs the batch to storage.
func (b *Batch) CommitAndSync() error {
	if !b.separated {
