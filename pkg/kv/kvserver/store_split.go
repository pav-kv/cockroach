...
func splitPreApply(
	...
	if rightRepl == nil || rightRepl.isNewerThanSplit(&split) {
		...
		if rightRepl != nil {
			...
			var err error
			hs, err = rightRepl.logStorage.raftMu.loader.LoadHardState(ctx, readWriter)
			if err != nil {
		...
		if rightRepl != nil {
			...
			if err := rightRepl.logStorage.raftMu.loader.SetHardState(ctx, readWriter, hs); err != nil {
	...
	if err := rsl.SynthesizeRaftState(
		ctx, logstore.NewStateLoader(split.RightDesc.RangeID, kvserverpb.TODOLogID), readWriter,
	); err != nil {
...
func prepareRightReplicaForSplit(
	...
	state, err := kvstorage.LoadReplicaState(
		ctx, r.store.TODOEngine(), r.StoreID(), &split.RightDesc,
		rightRepl.replicaID, rightRepl.logStorage.shMu.logID)
	if err != nil {
		log.Fatalf(ctx, "%v", err)
	}
