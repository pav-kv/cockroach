...
func (b *replicaAppBatch) runPostAddTriggersReplicaOnly(
	...
	if merge := res.Merge; merge != nil {
		...
		if err := kvstorage.DestroyReplica(
			ctx, rhsRepl.RangeID, rhsRepl.logStorage.shMu.logID,
			b.batch, b.batch, mergedTombstoneReplicaID,
	...
	if change := res.ChangeReplicas; change != nil &&
		...
		if err := kvstorage.DestroyReplica(
			ctx, b.r.RangeID, b.r.logStorage.shMu.logID,
			b.batch, b.batch, change.NextReplicaID(),
			kvstorage.ClearRangeDataOptions{
