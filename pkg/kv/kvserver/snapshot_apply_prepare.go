...
func prepareSnapApply(
	...
	_ = applySnapshotWrites // 3.1 + 1.1 + 2.5.
	...
	_ = applySnapshotWrites // add to 2.4.
	if err := input.writeSST(ctx, unreplicatedSSTFile.Data()); err != nil {
		return roachpb.Span{}, nil, err
	}

	_ = applySnapshotWrites // 3.2 + 2.1 + 2.2 + 2.3
...
func rewriteRaftState(
	...
	if err := sl.SetRaftReplicaID(ctx, w, kvserverpb.RaftReplicaID{
		ReplicaID: id.ReplicaID, LogID: kvserverpb.TODOLogIDRotate,
	}); err != nil {
