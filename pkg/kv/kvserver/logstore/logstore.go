...
type PebbleStats struct {
	...
	// CommitDur is the time it took to commit the batch to Pebble. If the write
	// is non-blocking, does not include waiting for sync.
...
func (s *LogStore) writeBatch(ctx context.Context, wb *writeBatch, cb SyncCallback) error {
	...
	wb.stats.Pebble.CommitDur = begin.Elapsed()
	wb.stats.Pebble.CommitStats = wb.batch.CommitStats()
	if wb.sd.wantsSync {
		cb.OnLogSync(ctx, wb.ack, WriteStats{CommitDur: wb.stats.Pebble.CommitDur})
	}
	return nil
}
...
func (s *LogStore) syncDecision(prevLast kvpb.RaftIndex, app raft.StorageAppend) syncDecision {
	...
	sd.nonBlocking = sd.willSync &&
		...
		// the overwritten entries. This must be performed after the corresponding
		// entries are durably replaced, and it's easier to do ensure proper
		// ordering using a blocking sync. This is a rare case, so it's not worth
		// optimizing for.
		!sd.overwriting &&
